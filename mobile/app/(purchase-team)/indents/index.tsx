...
export default function PendingIndents() {
    ...
    const loadIndents = async () => {
        try {
            ...
            const response = await fetch(`${API_URL}/indents?status=SUBMITTED`, {
    ...
    const handleApprove = async (id: string) => {
        try {
            const token = await SecureStore.getItemAsync('auth_access_token');
            const response = await fetch(`${API_URL}/indents/${id}/approve`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` },
            });
            if (response.ok) {
                Alert.alert('Success', 'Indent approved');
                loadIndents();
            }
        } catch (e) {
    ...
    const handleReject = async (id: string) => {
        Alert.prompt('Reject Indent', 'Enter reason:', [
            { text: 'Cancel', style: 'cancel' },
            {
                ...
                onPress: async (reason) => {
                    try {
                        const token = await SecureStore.getItemAsync('auth_access_token');
                        await fetch(`${API_URL}/indents/${id}/reject`, {
                            method: 'POST',
    ...
    const renderIndent = ({ item }: { item: any }) => (
        <View style={styles.card}>
            <TouchableOpacity onPress={() => router.push(`/(purchase-team)/indents/${item.id}`)}>
                <Text style={styles.indentNumber}>{item.indentNumber}</Text>
                <Text style={styles.indentName}>{item.name}</Text>
                <Text style={styles.meta}>{item.items?.length || 0} items • {item.site?.name || 'Site'}</Text>
            </TouchableOpacity>
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.title}>Pending Indents</Text>
                <View style={{ width: 50 }} />
            </View>
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<View style={styles.empty}><Text style={styles.emptyIcon}>✅</Text><Text style={styles.emptyText}>No pending indents</Text></View>}
                />
            )}
        </SafeAreaView>
    );
...
const styles = StyleSheet.create({
    ...
    indentNumber: { fontSize: 14, fontWeight: '600', color: '#8B5CF6' },
    indentName: { fontSize: 16, fontWeight: '600', color: '#111827', marginVertical: 4 },
