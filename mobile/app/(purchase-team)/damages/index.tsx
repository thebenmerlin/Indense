...
export default function DamagesReported() {
    ...
    const renderDamage = ({ item }: { item: DamageReport }) => {
        ...
        return (
            <TouchableOpacity
                onPress={() => router.push(`/(purchase-team)/damages/${item.id}` as any)}
                activeOpacity={0.7}
            >
                <View style={[styles.card, { borderLeftColor: severityColor }]}>
                    ...
                    <View style={styles.cardDetails}>
                        <View style={styles.detailRow}>
                            ...
                            <Text style={styles.detailText}>{item.site?.name}</Text>
                        </View>
                        <View style={styles.detailRow}>
...
const styles = StyleSheet.create({
    ...
    indentNumber: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 },
