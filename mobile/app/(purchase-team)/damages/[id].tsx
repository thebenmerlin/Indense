...
export default function DamageDetail() {
    ...
    const loadDamage = async () => {
        try {
            ...
            const response = await fetch(`${API_URL}/returns/damages/${id}`, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.content}>
                <View style={styles.card}>
                    ...
                    <View style={[styles.badge, { backgroundColor: damage?.severity === 'SEVERE' ? '#FEE2E2' : '#FEF3C7' }]}>
                        <Text style={[styles.badgeText, { color: damage?.severity === 'SEVERE' ? '#EF4444' : '#F59E0B' }]}>{damage?.severity}</Text>
                    </View>
                    <Text style={styles.label}>Description</Text>
                    <Text style={styles.value}>{damage?.description}</Text>
                    <Text style={[styles.label, { marginTop: 16 }]}>Damaged Quantity</Text>
                    <Text style={styles.value}>{damage?.damagedQty || 'N/A'}</Text>
